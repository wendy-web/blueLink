...
Page({
  ...
  getApplyStatus(){
    var that = this
    a.post("partner/apply_detail",{apply_type: 3}, function (data) {
      console.log(data)
      if(data.apply_record) {
        that.setData({
          ...
          shengfen: data.apply_record.idcard
        })
  ...
  inputshengfen:function(t){
    this.setData({
      shengfen: t.detail.value
    });
  },
  inputChange(t){
    this.setData({
    ...
    if (arrImg.length != 0) {
      ...
      c.myUploadimg(
        ...
        function (res) {
          ...
          e.post("partner/branch_office_apply", data, function (res) {
            console.log(res);
            wx.showToast({
              title: res.message,
              icon: 'success',
