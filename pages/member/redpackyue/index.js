...
Page({
  ...
  getlist:function(){
    var a = this;
    e.get('member/withdraw',{},function(res){
      if(res.error==0){
        a.setData({
          credit: res.credit,//可提现金额
          total_credit: res.total_credit,
          frozen_credit: res.frozen_credit,//待结算金额
          type_array: res.type_array ,
          last_data: res.last_data,//最后提现数据
          banklist: res.banklist,//银行数组
          withdrawcharge: res.withdrawcharge, // 提现手续费比例
          show:1
        })
      }
    })
  ...
  pickerChange: function (a) {
    ...
    e.checked = this.data.objectArray[t],
      3 == e.checked.id && (e.bankChecked = this.data.info.banklist[0], e.bankCheckedIndex = 0),
      this.setData(e)
      console.log(e);
  },
  ...
  bindmingxi:function(){
    wx.navigateTo({
      url: '/pages/member/yuemingxi/index',
    })
  },
  bindmingxi1:function(){
    wx.navigateTo({
      url: '/pages/member/yuemingxi/index?yj=1',
    })
  }
})